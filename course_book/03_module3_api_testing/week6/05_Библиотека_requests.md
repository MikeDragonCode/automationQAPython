...
- `response.content` — тело ответа как есть, в виде байтов (`bytes`), без декодирования в строку — пригодится для бинарных данных вроде файлов и картинок.
...
Тот же аргумент `json=` работает и для `put()`/`patch()` — синтаксис не меняется от метода к методу, меняется только то, что сервер сделает с телом (см. урок 3.2):
...
### raise_for_status() и обработка ошибок

У `Response` есть метод `raise_for_status()` — если статус-код ответа `4xx` или `5xx`, он бросит исключение `requests.exceptions.HTTPError`; если код `2xx`/`3xx`, ничего не произойдёт. Это удобно, когда важно сразу остановиться на ошибке, а не пытаться парсить тело несуществующего ответа:

```python
response = requests.get("https://jsonplaceholder.typicode.com/users/9999")
response.raise_for_status()  # бросит HTTPError, потому что статус 404
```

Кроме ошибок самого сервера, бывают ошибки на уровне сети — они возникают ещё до того, как появится какой-либо `response`, поэтому их нужно ловить отдельно, через `try/except`: `requests.exceptions.Timeout` (сервер не ответил за отведённое время) и `requests.exceptions.ConnectionError` (не удалось установить соединение вообще).
...
1. **Разминка.** Напиши один вызов `requests.get("https://jsonplaceholder.typicode.com/users/2")` без обёрток. Выведи в консоль `response.status_code` и `type(response.json())` — убедись, что тело действительно распарсилось в `dict`.
2. **Основное.** Напиши три отдельных вызова `requests` без обёрток: GET на `/users/1`, POST на `/posts` с телом `{"title": "test", "body": "text", "userId": 1}`, DELETE на `/posts/1`. Для каждого выведи в консоль и `response.status_code`, и `response.json()`. Убедись, что коды ответа соответствуют тому, что ты изучил в уроках 3.2–3.3.
3. **Ближе к практике.** Сделай GET на `/users/1`, получи `response.json()` и напиши несколько `assert`, проверяющих не только статус-код, но и форму ответа: что в словаре есть ключи `"id"`, `"name"`, `"email"`, `"address"`, и что `response.json()["id"] == 1`. Это то, что реально делает автотест — проверяет и код, и структуру данных.
4. **Со звёздочкой.** Выполни `requests.get("https://jsonplaceholder.typicode.com/guide/")` (это HTML-страница документации, а не JSON-эндпоинт) и оберни вызов `.json()` в `try/except`, поймав исключение, о котором предупреждает "Частые ошибки" этого урока. В `except`-блоке выведи понятное сообщение вместо падения программы — почувствуй на практике ту самую ошибку "response.json() на не-JSON теле".
