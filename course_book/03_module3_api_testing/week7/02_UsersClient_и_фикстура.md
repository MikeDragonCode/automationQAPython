...
### Что именно значит "один объект на всю сессию"

Если два разных теста в одном прогоне объявят `users_client: UsersClient` в параметрах, pytest подставит им **один и тот же** объект, а не два разных — фикстура вызывается один раз, а результат переиспользуется. Это можно увидеть напрямую (вспомни `id()` из модуля 0):

```python
def test_a(users_client: UsersClient):
    print(id(users_client))

def test_b(users_client: UsersClient):
    print(id(users_client))
    # id() в обоих тестах будет одинаковым — это один и тот же объект
```

Это безопасно именно потому, что `UsersClient` не хранит изменяемое состояние теста внутри `self` — единственное, что у него есть, это `session` для похода в сеть. Если бы клиент хранил, например, "последнего созданного пользователя" в атрибуте, `scope="session"` стал бы опасным — один тест мог бы неявно повлиять на другой через общий объект.

### Почему не scope="function" (значение по умолчанию)

Если бы у фикстуры `users_client` вообще не было указано `scope` (или было `scope="function"`, как у большинства фикстур по умолчанию), pytest создавал бы новый `UsersClient` — а значит, и новую `requests.Session` — перед каждым отдельным тестом. Для HTTP-клиента это не бесплатно: `requests.Session` умеет переиспользовать TCP-соединение между запросами (keep-alive), и пересоздание сессии на каждый тест теряет это преимущество. Раз клиент без состояния теста внутри — выгоднее собрать его один раз и переиспользовать.
...
1. **Разминка.** В отдельном скрипте создай `client = get_users_client()`, затем вызови `client.get_users_api()` и `client.get_user_api(user_id=3)`. Распечатай `response.status_code` и `len(response.json())` для первого вызова, и `response.json()["username"]` для второго. Не используй пока высокоуровневые методы — цель разминки в том, чтобы своими глазами увидеть, что `_api`-методы возвращают именно сырой `Response`.

2. **Основное.** Добавь в свой форк новый метод `find_user_by_username(self, username: str) -> UserSchema | None` в `UsersClient` — по тому же паттерну, что и остальные методы класса. Он должен вызвать уже существующий `get_users_api()`, пройтись по списку пользователей из `response.json()` и вернуть того, чей `username` совпадает с переданным, либо `None`, если такого нет. Чтобы превратить найденный словарь `matching_dict` в объект `UserSchema`, используй `UserSchema(**matching_dict)` — запись `**dict` распаковывает словарь в именованные аргументы конструктора модели (`id=..., name=..., username=...` и т.д.). Как это устроено внутри и какие ещё есть способы создать Pydantic-модель, подробно разберём в следующем уроке (3.9) — сейчас достаточно знать эту одну готовую конструкцию. Проверь метод в отдельном скрипте на реальном username с `https://jsonplaceholder.typicode.com/users` (например, `"Bret"`).

3. **Ближе к практике.** По тому же принципу добавь ещё один метод — `find_users_by_company_name(self, company_name: str) -> list[UserSchema]`. Он должен вызвать `get_users_api()`, пройтись по всем пользователям и вернуть список `UserSchema` для тех, у кого `company["name"]` совпадает с переданным (учти, что таких пользователей может быть и ноль — тогда возвращается пустой список, а не `None`, в отличие от задания 2, где ищется ровно один пользователь). Проверь на реальной компании из ответа `GET /users` (посмотри поле `company.name` у любого пользователя из списка).

4. **Со звёздочкой.** В отдельном скрипте создай два клиента напрямую: `client_1 = get_users_client()` и `client_2 = get_users_client()`, и сравни `id(client_1) == id(client_2)` — получится `False`, это два разных объекта с двумя разными HTTP-сессиями, потому что каждый вызов `get_users_client()` собирает клиента заново. Теперь напиши в форкнутом `boilerplate/` два маленьких теста, каждый со своим параметром `users_client: UsersClient` из фикстуры, и в обоих через `print(id(users_client))` (запусти `pytest -m api -v -s`, флаг `-s` нужен, чтобы `print` был виден) убедись, что id одинаковый. Объясни себе, почему в тестах, в отличие от твоего ручного скрипта, одинаковый `id` — это правильное поведение, а создание клиента вручную внутри каждого теста (частая ошибка выше) свело бы это преимущество фикстуры на нет.
