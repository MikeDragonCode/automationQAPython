...
### Группировка методов: сначала действия, потом проверки

Присмотрись к порядку методов в файле: `open_page`, `fill_registration_form`, `click_registration_button`, `check_visible_registration_form`. Обрати внимание, что в реальном тесте (`test_registration.py`) эти же методы вызываются в другом порядке: сначала `open_page()`, потом `check_visible_registration_form()`, и только потом `fill_registration_form()` и `click_registration_button()`. То есть порядок объявления методов в классе не копирует порядок их вызова в тесте — методы сгруппированы по типу (сначала все действия, затем проверка), а не по сценарию использования. Для Python это не имеет значения (класс работает одинаково при любом порядке методов), но для человека, читающего файл сверху вниз, такая группировка облегчает навигацию: если нужно найти проверку — она в конце, если действие — среди первых методов.

### Аннотации типов параметров — тоже документация, а не проверка

```python
def fill_registration_form(self, email: str, username: str, password: str) -> None:
```

`email: str`, `username: str`, `password: str` подсказывают и человеку, и IDE, что ожидается на входе. Но важно понимать границу: Python не проверяет эти типы во время выполнения. Если по ошибке передать `fill_registration_form(email=12345, username="user", password="pass")`, код не упадёт на этой строке — упадёт позже, внутри `.fill()`, потому что Playwright ожидает строку, а не число. Аннотация — это подсказка для читающего код и для автодополнения, а не защита от неправильного вызова.
...
1. **Разминка.** Объясни своими словами (или запиши комментарием), что именно сломается, если удалить строку `super().__init__(page)` из конструктора `RegistrationPage`. Какого конкретно атрибута не окажется, и какие методы класса от него зависят?
2. **Основное.** Найди в `boilerplate/pages/registration_page.py` строку, отвечающую за проверку видимости кнопки Registration, и объясни (себе или напарнику), почему она использует `expect(self.registration_button)`, а не, например, `expect(page.locator("[data-testid='registration-page-registration-button']"))` заново.
3. **Ближе к практике.** Не заглядывая в урок 4.16, попробуй самостоятельно (только на бумаге или в черновике, файл создавать не нужно) написать конструктор гипотетического `LoginPage`, у которого форма устроена так же, как у `RegistrationPage`: два поля-обёртки (email, password), каждое с вложенным `input`, и кнопка, `data-testid` которой стоит прямо на элементе. Используй ровно тот же шаблон: `super().__init__(page)`, затем локаторы в `self.*`. Сверишь результат на уроке 4.16.
4. **Со звёздочкой.** Представь, что кто-то объединил `fill_registration_form` и `click_registration_button` в один метод `complete_registration(email, username, password)`, который заполняет форму и сразу кликает. Возьми негативный сценарий из домашнего задания модуля — "регистрация с уже занятым email" (нужно зарегистрировать пользователя дважды с одним email, но разными username) — и объясни, почему такой объединённый метод усложнит написание именно этого теста.
