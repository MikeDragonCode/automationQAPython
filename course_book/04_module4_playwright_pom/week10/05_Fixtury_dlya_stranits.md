...
### Scope фикстуры: почему registration_page создаётся заново для каждого теста

В том же `conftest.py` есть фикстура из модуля 3, у которой явно указан scope:
...
А у `registration_page` и `dashboard_page` scope не указан вовсе — значит, используется значение по умолчанию, `function`: объект создаётся заново для каждого теста. Разница не случайна. `page` от `pytest-playwright`, от которого зависят `registration_page` и `dashboard_page`, тоже создаётся заново на каждый тест — это отдельная вкладка браузера, и тесты не должны делить между собой состояние страницы (куки, введённые данные, историю переходов). Поэтому Page Object, обёрнутый вокруг `page`, обязан быть настолько же "свежим", насколько свежа сама вкладка. `users_client`, напротив, не имеет отношения к браузеру — это просто HTTP-клиент, и его можно безопасно переиспользовать на весь тестовый прогон (`scope="session"`), не пересоздавая ради экономии.

### Порядок параметров в тесте не важен
...
pytest подставляет фикстуры по имени параметра, а не по позиции. Поэтому `user_data`, `registration_page` и `dashboard_page` можно перечислить в сигнатуре теста в любом порядке — результат будет одинаковым. Это удобно помнить, когда добавляешь новую фикстуру (например, `login_page` в домашнем задании) в уже существующий тест: не нужно думать, "перед" какой фикстурой её вставить.
...
1. **Разминка.** Открой `boilerplate/conftest.py` и выпиши рядом друг с другом сигнатуры фикстур `users_client` и `registration_page`. Обведи, чем отличается их `@pytest.fixture(...)` декоратор, и назови, какое значение scope у каждой (явное или по умолчанию).
2. **Основное.** Открой `boilerplate/conftest.py` и найди фикстуру `user_data`. Определи, зависит ли она от фикстуры `page` так же, как `registration_page` и `dashboard_page`, или устроена независимо. Обоснуй разницу тем, что именно возвращает каждая из фикстур.
3. **Ближе к практике.** Домашнему заданию понадобится фикстура `login_page` в `conftest.py`, по аналогии с `registration_page`/`dashboard_page`. Не открывая урок 4.16, напиши (только текстом, файл менять не нужно) её полную сигнатуру и тело, основываясь исключительно на шаблоне из этого урока.
4. **Со звёздочкой.** Предположим, кто-то оформил фикстуру `login_page` так, что она сама открывает страницу: `@pytest.fixture def login_page(page): p = LoginPage(page); p.open_page(); return p`. Объясни, какой конкретный сценарий это сломает или сделает неудобным — вспомни, что `registration_page.open_page()` в реальном тесте вызывается явно, внутри `allure.step(...)`, как отдельный видимый шаг сценария.
